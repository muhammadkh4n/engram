...
export interface LoCoMoTurn {
  ...
  text: string
  /**
   * Optional pre-computed hypothetical questions this turn could answer.
   * Generated by `preprocess-hypothetical-qs.ts` (Phase 5 ingestion fix).
   * When `LoCoMoAdapter.ingestConversation` is called with
   * `withHypotheticalQuestions: true`, each HQ is dual-ingested as an
   * augmented chunk sharing the same `dia_id` for recall@K scoring.
   */
