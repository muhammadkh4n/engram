...
describe('CircuitBreaker', () => {
  ...
  describe('half-open state', () => {
    ...
    it('SEC4: only one concurrent request proceeds in half-open, others fail fast', async () => {
      const fastBreaker = new CircuitBreaker({ threshold: 1, cooldownMs: 50 });
      // Open the circuit.
      try { await fastBreaker.execute(() => Promise.reject(new Error('fail'))); } catch {}
      ...
      // Launch two concurrent calls while in half-open.
      let probeStarted = false;
      const slowProbe = fastBreaker.execute(
        () =>
      ...
      // Give the first call time to grab the probe slot.
      await new Promise((r) => setTimeout(r, 5));

      // A second concurrent call should be rejected immediately.
      await expect(
        fastBreaker.execute(() => Promise.resolve('should not run'))
      ).rejects.toBeInstanceOf(CircuitOpenError);

      // The first probe should still succeed.
      await expect(slowProbe).resolves.toBe('ok');
